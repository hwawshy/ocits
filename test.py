...
from torch.nn import ModuleList, Conv1d, MaxPool1d, BCEWithLogitsLoss
from torch_geometric.data import Data, InMemoryDataset
import numpy as np
import pandas as pd
import torch.nn.functional as F
from torch_geometric.loader import DataLoader
from torch_geometric.nn import global_sort_pool, GCNConv, MLP
from torch_geometric.transforms import RandomLinkSplit

from torch_geometric.utils import k_hop_subgraph, to_scipy_sparse_matrix


# based on https://stackoverflow.com/a/64597531
def pajek_to_data(path: str) -> Data:
    # Load into RAM
    ...
    _, vertex_text, _, edges_text = re.split(r'\*[^\n]+\n', full_text)

    # Parse vertices
    v_df = pd.read_csv(StringIO(vertex_text), delimiter=' ', engine='c', names=['id', 'label', 'f1', 'f2', 'f3'], header=None)
    assert (v_df['id'] == np.arange(1, 1 + num_vertices)).all(), \
        "File does not list all vertices, or lists them out of order."

    # Parse edges
    e_df = pd.read_csv(StringIO(edges_text), delimiter=' ', engine='c', names=['v1', 'v2', 'weight'], header=None)
    ...
    edge_index = torch.tensor(e_df.loc[:, ['v1', 'v2']].values.tolist(), dtype=torch.long)
    edge_attr = torch.tensor(e_df.loc[:, ['weight']].values.tolist(), dtype=torch.float)
    x = torch.tensor(v_df.loc[:, ['f1', 'f2', 'f3']].values.tolist(), dtype=torch.float)

    return Data(x=x, edge_index=edge_index.t().contiguous(), edge_attr=edge_attr)


# source: https://github.com/pyg-team/pytorch_geometric/blob/master/examples/seal_link_pred.py
class SEALDataset(InMemoryDataset):
    def __init__(self, root, data, num_hops, split='train'):
        self.data = data
    ...
    def process(self):
        transform = RandomLinkSplit(num_val=0.05, num_test=0.2,
                                    is_undirected=False, split_labels=True, add_negative_train_samples=True)
...
data = pajek_to_data(osp.join(root, 'raw', 'USAir97.net.txt'))

train_dataset = SEALDataset(data=data, root=root, num_hops=2, split='train')
val_dataset = SEALDataset(data=data, root=root, num_hops=2, split='val')
test_dataset = SEALDataset(data=data, root=root, num_hops=2, split='test')
